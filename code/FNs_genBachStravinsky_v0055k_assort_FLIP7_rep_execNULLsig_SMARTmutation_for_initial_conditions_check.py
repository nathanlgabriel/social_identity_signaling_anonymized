...
@numba.jit(nopython=True, fastmath=False, parallel=False)
def genBSreplicatorexec_k_step_initcon(init_conf, numtypesf, rngf, population_sizef, numprofilesf, poptf):
    # going to hard code the assignment of appropriate profiles given the desired initial conditions
    # this hard coding is specific to the model with three types and three signals
    # repmultipliers for 3x3x3 game is [ 1  3  9 27]
    # so profile for no signal allways do action 0 is 1*0+3*0+9*0+27*0 =0
    # profile for no signal and always do action 1 is 1*0+3*1+9*1+27*1 = 39
    # profile for no signal and always do action 2 is 1*0+3*2+9*2+27*2 = 78
    pref_profiles = np.array([0, 39, 78])
    ...
    for idx1100 in range(0, population_sizef):
        tdex = poptf[idx1100]
        typestotal[tdex] += 1
        if init_conf == "preference":
            kprofile = pref_profiles[tdex]
        else:
            kprofile = 78
        agentprofiles[idx1100] = kprofile
        profile_count_typed[tdex][kprofile] += 1
...
@numba.jit(nopython=True, fastmath=False, parallel=False)
def genBSreplicatorexec_full_play(init_conf, mutateratef, sigcostf, repmultipliersf, numprofilesf, signal_snapshotsf, record_intervalf, genBSpunishf, numtypesf, numsignalsf, runlengthf, numactionsf, coordination_preferencesf, poptf, sigurnsf, recurnsf, population_sizef, sigdimensionsf, base_connection_weightsf, sigmultipliersf, homophily_factorf, rngf, epsilonf, runid):
    typed_time = np.zeros((numtypesf, numsignalsf, numactionsf, runlengthf//record_intervalf), dtype=np.float64)
    signal_time = np.zeros((numsignalsf, numsignalsf, numactionsf, runlengthf//record_intervalf), dtype=np.float64)
    typed_time_norm = np.zeros((numtypesf, numsignalsf, numactionsf, runlengthf//record_intervalf), dtype=np.float64)
    signal_time_norm = np.zeros((numsignalsf, numsignalsf, numactionsf, runlengthf//record_intervalf), dtype=np.float64)
...
#     typestotal, profile_count_typed, profile_count_untyped, agentprofiles = genBSreplicatorexec_first_tstep(sigcostf, sigperdimf, repmultipliersf, genBSpunishf_iter, numsignalsf, numprofilesf, numtypesf, numactionsf, coordination_preferencesf, poptf, sigurnsf, recurnsf, execurnsf, population_sizef, sigdimensionsf, base_connection_weightsf, sigmultipliersf, homophily_factorf, profiles_indexed)
    typestotal, profile_count_typed, profile_count_untyped, agentprofiles = genBSreplicatorexec_k_step_initcon(init_conf, numtypesf, rngf, population_sizef, numprofilesf, poptf)
    agentprofilesCOPY = agentprofiles.copy()
    mutations10, newprofiles10, muteprofilestrings10 = muterandoms10_smart(rngf, mutateratef, population_sizef, numprofilesf, len(repmultipliersf))
    mute_idxN = 0
    profile_count_typed, profile_count_untyped = repmutation_smart(repmultipliersf, profilecaps, profiles_indexed, numtypesf, numprofilesf, profile_count_typed, profile_count_untyped, mutations10[mute_idxN], newprofiles10[mute_idxN], muteprofilestrings10[mute_idxN])
...
def genBSreplicatorexec_full_play_typed(init_conf, mutateratef, sigcostf, repmultipliersf, numprofilesf, signal_snapshotsf, record_intervalf, genBSpunishf, numtypesf, numsignalsf, runlengthf, numactionsf, coordination_preferencesf, poptf, sigurnsf, recurnsf, population_sizef, sigdimensionsf, base_connection_weightsf, sigmultipliersf, homophily_factorf, rngf, epsilonf, runid):
    sigurnsf = numba.typed.List(sigurnsf)
    simple_stat_002, simple_stat_001, socialsig_aggregate, action_aggregate, sigurnsf, recurnsf, execurnsf, typed_time, signal_time, typed_time_norm, signal_time_norm, runid = genBSreplicatorexec_full_play(init_conf, mutateratef, sigcostf, repmultipliersf, numprofilesf, signal_snapshotsf, record_intervalf, genBSpunishf, numtypesf, numsignalsf, runlengthf, numactionsf, coordination_preferencesf, poptf, sigurnsf, recurnsf, population_sizef, sigdimensionsf, base_connection_weightsf, sigmultipliersf, homophily_factorf, rngf, epsilonf, runid)
    sigurns_untyped = []
    for surn in sigurnsf:
        sigurns_untyped.append(np.array(surn))
